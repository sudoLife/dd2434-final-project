...
class EdgeMinibatchIterator(object):
    ...
    def construct_adj(self):
        adj = len(self.id2idx)*np.ones((len(self.id2idx)+1, self.max_degree), dtype = tf.int32)
...
class NodeMinibatchIterator(object):
    ...
    minibatch = NodeMinibatchIterator(G, 
            ...
            context_pairs = context_pairs) in train_supervised.py
    ...
    def construct_adj(self):
        ...
        # G.nodes(): contains the ids of all nodes 

        for nodeid in self.G.nodes():
            if self.G.nodes[nodeid]['test'] or self.G.nodes[nodeid]['val']:
                continue
            neighbors = np.array([self.id2idx[str(neighbor)] 
                for neighbor in self.G.neighbors(nodeid) # id of the neighbors of node 
                if (not self.G[nodeid][neighbor]['train_removed'])]) # only keep edges used for train 
