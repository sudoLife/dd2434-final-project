...
def load_data(prefix, normalize=True, load_walks=False):
    ...
    for edge in G.edges():
        #edge[0]: target // edge[1]:context 
        if (G.nodes[edge[0]]['val'] or G.nodes[edge[1]]['val'] or
            G.nodes[edge[0]]['test'] or G.nodes[edge[1]]['test']):
            # if one of the nodes that constitue the edge is used for validation 
            # or test that, then the edge is not used for train 
            # otherwise it is used for train. 
