...
from SAGEmodels import SupervisedGraphsage
from SAGEmodels import SAGEInfo
...
flags.DEFINE_string('train_prefix', 'toy-ppi', 'prefix identifying training data. must be specified.')
...
def log_dir():
    # return the name of the directory for logging and saving embeddings
...
def train(train_data, test_data=None):
    ...
    G = train_data[0]
    # to get the attribute of G, uncomment the line below:
    # print(dir(G))
    features = train_data[1] # a numpy array of features, ordering given by id_map.json. 
    # Can be omitted and only identity features will be used.
    id_map = train_data[2]  # a dict mapping node ids to consecutive integers 
    class_map  = train_data[4] # a dict mapping node ids to classes 
    
    # train_data[3] is supposed to be walks variable, which we are not using in the 
    # supervised learning 
    ...
    if isinstance(list(class_map.values())[0], list):
        ...
        num_classes = len(list(class_map.values())[0]) # 121 classes 
    else:
    ...
    adj_info_ph = tf.compat.v1.placeholder(np.int32, shape=minibatch.adj.shape) #HERE
    ...
    for epoch in range(FLAGS.epochs): 
        ...
        while not minibatch.end():
            ...
            feed_dict, labels = minibatch.next_minibatch_feed_dict() # HERE
