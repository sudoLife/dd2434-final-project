...
default_params = {
    ...
    "prop_reversed_neg": 0.5  # How many percent of the edges in the test set is reversed to be negative samples
    # 3 cases: 0, 0.5, 1
}
...
class GraphEmbedding():
    def __init__(self, nx_G=None, is_directed=False,
                 prop_pos=0.5, prop_neg=0.5, workers=1, random_seed=None, prop_reversed_neg=0.5):
        ...
        self.wvecs = None
        self.embeddings = None  # embeddings should be ordered and start the index of 0
    ...
    def netmf_train_embeddings(self):
        """
        load the embedding here to be self.embeddings
        embeddings should be ordered and start the index of 0
        """
    ...
    def deepwalk_train_embeddings(self, window=10, walk_length=40, walks_per_vertex=80, embedding_size=128):
        ...
        X = np.zeros(deepwalk_model.vectors.shape)
        for i, k in enumerate(deepwalk_model.key_to_index):
            k = int(k)
            X[k - 1] = deepwalk_model.vectors[i]
    ...
    def node2vec_train_embeddings(self, window=10, walk_length=40, walks_per_vertex=80, embedding_size=128, p=4,
        ...
        X = np.zeros(node2vec_model.vectors.shape)
        for i, k in enumerate(node2vec_model.key_to_index):
            k = int(k)
            X[k - 1] = node2vec_model.vectors[i]
    ...
    def generate_pos_neg_links(self):
        ...
        Modify graph by removing the positive links.
        """
        ...
        if self.is_directed:
            ...
            random.shuffle(pos_edge_list)
            # take the first prop_reversed_neg * len(pos_edge_list) , iterate through it
            n_reverse_edge = int(self.prop_reversed_neg * len(pos_edge_list))
            pos_edges_to_be_reversed = pos_edge_list[:n_reverse_edge]
            for i, e in enumerate(pos_edges_to_be_reversed):
                # if the reversed edge is not in the pos_edge_list
                if e not in pos_edge_list:
                    # replace an edge in the neg_edge_list with it
                    neg_edge_list[i] = tuple(reversed(e))
...
def create_train_test_graphs(args, graph):
    ...
    cached_fn = "eu_email_directed_DeepWalk_LP_1.graph"
    if os.path.exists(cached_fn) and not args.regen:
        ...
        # For test graph, we'll need to get a portion of negative samples to be the reverse of a positive one
...
def test_edge_functions(args, graph):
    ...
    for iter in range(args.num_experiments):
        ...
        Gtrain.netmf_train_embeddings()
        Gtest.netmf_train_embeddings()
...
if __name__ == "__main__":
    ...
    nx.relabel_nodes(G, lambda x: x - 1, copy=False)
